...
const phases = {
  ...
  dev:    {namespace:'txkggj-dev'   , name: `${name}`, phase: 'dev'   , changeId:changeId, suffix: `-dev-${changeId}`   , instance: `${name}-dev-${changeId}`   , version:`${version}-${changeId}`  , tag:`dev-${version}-${changeId}`  , host: `hmr-${changeId}-txkggj-dev.pathfinder.gov.bc.ca` , dotnet_env: 'Development', transient: true},
  test:   {namespace:'txkggj-test'  , name: `${name}`, phase: 'test'  , changeId:changeId, suffix: `-test`              , instance: `${name}-test`              , version:`${version}`              , tag:`test-${version}`             , host: `hmr-txkggj-test.pathfinder.gov.bc.ca`            , dotnet_env: 'Staging'},
