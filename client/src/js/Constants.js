export const API_URL = window.RUNTIME_VITE_API_HOST //In non-dev environments, start.sh makes sure the runtime variables are popluated using host environment variables.
  ? `${window.location.protocol}//${window.RUNTIME_VITE_API_HOST}/api` //For non-dev environments, CORS is enabled to HMCR urls.
  : `${window.location.protocol}//${process.env.VITE_CLIENT_ORIGIN}/api`; //For dev environment, proxy (setupProxy.js) is set up to avoid the same origin policy
...
export const DEFAULT_PAGE_SIZE_OPTIONS = process.env.VITE_DEFAULT_PAGE_SIZE_OPTIONS.split(',').map((o) =>
  parseInt(o)
);

export const DEFAULT_PAGE_SIZE = parseInt(process.env.VITE_DEFAULT_PAGE_SIZE);
