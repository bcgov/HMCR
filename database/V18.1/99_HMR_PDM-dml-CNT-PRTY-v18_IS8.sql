...
-- Updates: 
--	2020-03-04: Support of Sprint 8.  
...
INNER JOIN (SELECT PARTY_ID, CONTRACT_NAME FROM (
SELECT 
	  PRTY.PARTY_ID
	  ,RANK() OVER (PARTITION BY SRV_ARA.[SERVICE_AREA_NUMBER] ORDER BY SRV_ARA_USR.APP_LAST_UPDATE_TIMESTAMP DESC) AS Rank
	  ,cnrt_trm.CONTRACT_NAME
  FROM [dbo].[HMR_SERVICE_AREA] SRV_ARA
  INNER JOIN [dbo].[HMR_SERVICE_AREA_USER] SRV_ARA_USR ON SRV_ARA.SERVICE_AREA_NUMBER = SRV_ARA_USR.SERVICE_AREA_NUMBER
  INNER JOIN [dbo].[HMR_SYSTEM_USER] SYS_USR ON SRV_ARA_USR.SYSTEM_USER_ID = SYS_USR.SYSTEM_USER_ID
  INNER JOIN [dbo].[HMR_PARTY] PRTY ON SYS_USR.PARTY_ID = PRTY.PARTY_ID
  INNER JOIN [dbo].[HMR_CONTRACT_TERM] cnrt_trm ON cnrt_trm.SERVICE_AREA_NUMBER = SRV_ARA_USR.SERVICE_AREA_NUMBER
  WHERE SYS_USR.USER_TYPE = 'BUSINESS' AND SYS_USR.BUSINESS_GUID IS NOT NULL AND PRTY.BUSINESS_GUID IS NOT NULL
) bceid_pty 
WHERE rank = 1) prty_cnrt ON CNRT_TRM.CONTRACT_NAME = prty_cnrt.CONTRACT_NAME
GO
