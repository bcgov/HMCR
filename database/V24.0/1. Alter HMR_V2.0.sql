// Script generated by Aqua Data Studio Schema Synchronization for MS SQL Server 2016 on Thu Oct 01 16:38:26 PDT 2020
// Execute this script on:
// 		HMR_1.3/dbo - This database/schema will be modified
...
// Create table dbo.HMR_ACTIVITY_RULE
PRINT N'Create table dbo.HMR_ACTIVITY_RULE'
GO
CREATE TABLE [dbo].[HMR_ACTIVITY_RULE]  ( 
	[ACTIVITY_RULE_ID]              	numeric(9,0) NOT NULL DEFAULT (NEXT VALUE FOR [HMR_ACTIVITY_RULE_CODE_ID_SEQ]),
	[ACTIVITY_RULE_SET]             	varchar(20) NOT NULL,
	[ACTIVITY_RULE_NAME]            	varchar(150) NOT NULL,
	...
	PRIMARY KEY CLUSTERED([ACTIVITY_RULE_ID])
 ON [PRIMARY])
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO
...
GO
CREATE TABLE [dbo].[HMR_ACTIVITY_CODE_RULE]  ( 
	[ACTIVITY_CODE_RULE_ID]         	numeric(9,0) NOT NULL DEFAULT (NEXT VALUE FOR [HMR_ACTIVITY_RULE_ID_SEQ]),
	[ACTIVITY_CODE_ID]              	numeric(9,0) NOT NULL,
	[ACTIVITY_RULE_ID]              	numeric(9,0) NOT NULL,
	[CONCURRENCY_CONTROL_NUMBER]    	bigint NOT NULL DEFAULT ((1)),
	[DB_AUDIT_CREATE_USERID]        	varchar(30) NOT NULL DEFAULT (user_name()),
	[DB_AUDIT_CREATE_TIMESTAMP]     	datetime NOT NULL DEFAULT (getutcdate()),
	[DB_AUDIT_LAST_UPDATE_USERID]   	varchar(30) NOT NULL DEFAULT (user_name()),
...
// Create unique constraint dbo.HMR_ACT_CD_RL_UK
PRINT N'Create unique constraint dbo.HMR_ACT_CD_RL_UK'
GO
ALTER TABLE [dbo].[HMR_ACTIVITY_CODE_RULE]
	ADD UNIQUE ([ACTIVITY_RULE_ID], [ACTIVITY_CODE_ID])
	WITH (
...
// Create index dbo.HMR_ACT_CD_RL_ACTV_RL_ID_FK_I
PRINT N'Create index dbo.HMR_ACT_CD_RL_ACTV_RL_ID_FK_I'
GO
CREATE NONCLUSTERED INDEX [HMR_ACT_CD_RL_ACTV_RL_ID_FK_I]
	ON [dbo].[HMR_ACTIVITY_CODE_RULE]([ACTIVITY_RULE_ID])
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO

// Create index dbo.HMR_ACT_CD_RL_ACTV_CD_ID_FK_I
PRINT N'Create index dbo.HMR_ACT_CD_RL_ACTV_CD_ID_FK_I'
GO
CREATE NONCLUSTERED INDEX [HMR_ACT_CD_RL_ACTV_CD_ID_FK_I]
	ON [dbo].[HMR_ACTIVITY_CODE_RULE]([ACTIVITY_CODE_ID])
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO

// Create foreign key constraint dbo.HMR_ACT_CD_RL_ACT_RUL_FK
PRINT N'Create foreign key constraint dbo.HMR_ACT_CD_RL_ACT_RUL_FK'
GO
ALTER TABLE [dbo].[HMR_ACTIVITY_CODE_RULE]
	ADD FOREIGN KEY([ACTIVITY_RULE_ID])
	REFERENCES [dbo].[HMR_ACTIVITY_RULE]([ACTIVITY_RULE_ID])
	ON DELETE NO ACTION 
	ON UPDATE NO ACTION 
GO
IF @@ERROR <> 0 SET NOEXEC ON
...
// Create foreign key constraint dbo.HMR_ACT_CODE_ACT_CD_RL_FK
...
// Create trigger dbo.HMR_ACT_RUL_I_S_U_TR
PRINT N'Create trigger dbo.HMR_ACT_RUL_I_S_U_TR'
GO
CREATE TRIGGER [dbo].[HMR_ACT_RUL_I_S_U_TR] ON HMR_ACTIVITY_RULE INSTEAD OF UPDATE AS
SET NOCOUNT ON
BEGIN TRY
  ...
  if exists (select 1 from inserted, deleted where inserted.CONCURRENCY_CONTROL_NUMBER != deleted.CONCURRENCY_CONTROL_NUMBER+1 AND inserted.ACTIVITY_RULE_ID = deleted.ACTIVITY_RULE_ID)
    raiserror('CONCURRENCY FAILURE.',16,1)


  -- update statement
  update HMR_ACTIVITY_RULE
    set "ACTIVITY_RULE_ID" = inserted."ACTIVITY_RULE_ID",
    ...
    from HMR_ACTIVITY_RULE
    inner join inserted
    on (HMR_ACTIVITY_RULE.ACTIVITY_RULE_ID = inserted.ACTIVITY_RULE_ID);
...
BEGIN TRY
  ...
  update HMR_ACTIVITY_CODE_RULE
    set "ACTIVITY_CODE_RULE_ID" = inserted."ACTIVITY_CODE_RULE_ID",
      ...
      "ACTIVITY_RULE_ID" = inserted."ACTIVITY_RULE_ID",
      "CONCURRENCY_CONTROL_NUMBER" = inserted."CONCURRENCY_CONTROL_NUMBER"
    , DB_AUDIT_LAST_UPDATE_TIMESTAMP = getutcdate()
    , DB_AUDIT_LAST_UPDATE_USERID = user_name()
...
// Create trigger dbo.HMR_ACT_RUL_I_S_I_TR
PRINT N'Create trigger dbo.HMR_ACT_RUL_I_S_I_TR'
GO
CREATE TRIGGER [dbo].[HMR_ACT_RUL_I_S_I_TR] ON HMR_ACTIVITY_RULE INSTEAD OF INSERT AS
SET NOCOUNT ON
BEGIN TRY
