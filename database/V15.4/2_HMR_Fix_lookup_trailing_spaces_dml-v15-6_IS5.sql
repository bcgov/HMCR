...
UPDATE [dbo].[HMR_ACTIVITY_CODE]
   ...
   WHERE EXISTS (Select 1 FROM [dbo].[HMR_CODE_LOOKUP] WHERE CODE_SET = 'WRK_RPT_MAINT_TYPE' AND CODE_NAME = MAINTENANCE_TYPE)
	 
UPDATE [dbo].[HMR_ACTIVITY_CODE]
   SET UNIT_OF_MEASURE = 'litre', CONCURRENCY_CONTROL_NUMBER=CONCURRENCY_CONTROL_NUMBER+1, APP_LAST_UPDATE_USERID=@app_user,APP_LAST_UPDATE_TIMESTAMP=@utcdate,APP_LAST_UPDATE_USER_GUID=@app_guid,APP_LAST_UPDATE_USER_DIRECTORY=@app_user_dir
   WHERE UNIT_OF_MEASURE = 'litres'
